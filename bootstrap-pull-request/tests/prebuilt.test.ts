...
describe('syncServicesFromPrebuilt', () => {
  ...
  it('preserves an application manifest if it was pushed on the current commit', async () => {
    const namespaceDirectory = await createEmptyDirectory()
    await fs.mkdir(`${namespaceDirectory}/applications`)
    await fs.writeFile(`${namespaceDirectory}/applications/pr-123--outdated.yaml`, applicationPushedOnCurrentCommit)
