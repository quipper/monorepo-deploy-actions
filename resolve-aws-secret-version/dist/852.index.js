...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, parentClientConfig, callerClientConfig, profile, filepath, configFilepath, ignoreCache, logger, }) => {
    ...
    const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(function () { return __webpack_require__(7903); });
    const sso = ssoClient ||
...
var sso = __webpack_require__(131);
...
const endpointResolver_1 = __webpack_require__(2274);
const schemas_0_1 = __webpack_require__(4119);
const getRuntimeConfig = (config) => {
    return {
...
const getSsoOidcClient = async (ssoRegion, init = {}, callerClientConfig) => {
    const { SSOOIDCClient } = await __webpack_require__.e(/* import() */ 683).then(__webpack_require__.t.bind(__webpack_require__, 5683, 23));
...
const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}, callerClientConfig) => {
    const { CreateTokenCommand } = await __webpack_require__.e(/* import() */ 683).then(__webpack_require__.t.bind(__webpack_require__, 5683, 23));
